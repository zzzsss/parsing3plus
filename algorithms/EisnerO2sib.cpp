...
vector<int>* decodeProjective_o2sib(int length,OneScores<REAL_SCORES>& scores)
{
	// the tables
	int total_size = length*length*2*3;
	REAL_SCORES* scores_table = new REAL_SCORES[total_size];
	...
	for(int k=1; k<length; k++){
		...
		for(int s=0; s<length; s++){
			...
			split_point = -2;
			for(int r = s; r < t; r++){
				REAL_SCORES tmp = scores_table[get_index_o2sib(length,s,r,E_RIGHT_O2sib,E_COM_O2sib)]
			...
			//others
			for(int r=s+1;r<t;r++){
				REAL_SCORES tmp = scores_table[get_index_o2sib(length,s,r,0,E_SIB_O2sib)]
			...
			//others
			for(int r=s+1;r<t;r++){
				REAL_SCORES tmp = scores_table[get_index_o2sib(length,r,t,0,E_SIB_O2sib)]
			...
			split_point = -2;
			for(int r = s; r < t; r++){
				REAL_SCORES tmp = scores_table[get_index_o2sib(length,s,r,E_LEFT_O2sib,E_COM_O2sib)]
				                          +scores_table[get_index_o2sib(length,r,t,E_LEFT_O2sib,E_INCOM_O2sib)];
				if(tmp >= max_score){
					max_score = tmp;
					split_point = r;
			...
			split_point = -2;
			for(int r = s+1; r <= t; r++){
				REAL_SCORES tmp = scores_table[get_index_o2sib(length,s,r,E_RIGHT_O2sib,E_INCOM_O2sib)]
