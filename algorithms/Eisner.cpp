...
//#define REAL_SCORES int
vector<int>* decodeProjective(int length,OneScores<REAL_SCORES>& scores)
{
	// the tables
	...
	for(int k=1; k<length; k++){
		...
		for(int s=0; s<length; s++){
			...
			u = -2;
			for(int r = s; r < t; r++){
				REAL_SCORES tmp = scores_table[get_index(length,s,r,E_RIGHT,E_COM)]
			...
			u = -2;
			for(int r = s; r < t; r++){
				REAL_SCORES tmp = scores_table[get_index(length,s,r,E_LEFT,E_COM)]
			...
			u = -2;
			for(int r = s+1; r <= t; r++){
				REAL_SCORES tmp = scores_table[get_index(length,s,r,E_RIGHT,E_INCOM)]
